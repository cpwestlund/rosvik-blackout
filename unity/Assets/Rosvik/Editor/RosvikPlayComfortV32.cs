    ...
    public static class RosvikPlayComfortV32 {
        ...
        static List<RoadSample> RoadSamples(List<RosvikOsmV15.Way> ways, Vector3 center, float radius) {
            ...
            foreach (var w in ways) {
                string h = w.Tag("highway"); int kind;
                ...
                for (int i=0;i<w.Nodes.Count-1;i++) {
                    Vector3 a=w.Nodes[i].Pos, b=w.Nodes[i+1].Pos, p=(a+b)*.5f, dir=Flat(b-a);
        ...
        static void AddUtilityDetails(Transform parent, List<RoadSample> roads, List<RosvikOsmV15.Way> ways, Vector3 school,
            ...
            if (dumpster) {
                foreach (RoadSample s in roads.Where(r=>r.kind==1).OrderBy(r=>Mathf.Abs(r.distance-42f))) {
                    ...
                    PlaceModel(dumpster,parent,"service dumpster",p,Yaw(s.dir),1.28f,utility); break;
                }
            }
            if (trash) {
                int made=0;
                foreach (RoadSample s in roads.Where(r=>r.kind==0).OrderBy(r=>r.distance)) {
                    ...
                    PlaceModel(trash,parent,"path waste bin",p,Yaw(s.dir),.72f,binMat); made++;
                }
            }
        }
        ...
        static void AddRoadsideWetness(Transform parent, List<RoadSample> roads, List<RosvikOsmV15.Way> ways, Material wet) {
            ...
            foreach (RoadSample s in roads.Where(r=>r.kind==1 || r.kind==2).OrderBy(r=>r.distance)) {
                ...
                if (InsideBuilding(p,ways) || InsidePitch(p,ways)) continue;
                Puddle(parent,p,Yaw(s.dir)+(made%3-1)*12f,Mathf.Lerp(.55f,1.15f,(made%4)/3f),Mathf.Lerp(.18f,.35f,(made%3)/2f),wet); made++;
            }
        }
        ...
        static void AddOuterVegetation(Transform parent, Transform root, List<RosvikOsmV15.Way> ways, Vector3 school,
            ...
            System.Random rng=new System.Random(3209); int trees=0,bushes=0;
            for (int attempt=0; attempt<700 && trees<26; attempt++) {
                float a=(float)rng.NextDouble()*Mathf.PI*2f, r=42f+(float)rng.NextDouble()*58f;
            ...
            if (bush) {
                for (int attempt=0; attempt<600 && bushes<38; attempt++) {
                    float a=(float)rng.NextDouble()*Mathf.PI*2f, r=34f+(float)rng.NextDouble()*67f;
                    Vector3 p=school+new Vector3(Mathf.Cos(a)*r,.035f,Mathf.Sin(a)*r);
                    if (!SafeForNature(p,ways,1.5f)) continue;
                    PlaceModel(bush,parent,"v32 outer shrub",p,(float)rng.NextDouble()*360f,Mathf.Lerp(.42f,.95f,(float)rng.NextDouble()),shrub); bushes++;
                }
            }
        }
        ...
        static void TuneCamera(Transform player) {
            ...
            Rosvik.Blackout.IsometricCameraRig rig=cam.GetComponent<Rosvik.Blackout.IsometricCameraRig>(); if (!rig) return;
            if (player) rig.target=player;
            rig.defaultSize=8.15f; rig.minSize=5.2f; rig.maxSize=19.5f; rig.zoomStep=.80f; rig.keyboardZoomSpeed=5.5f; rig.zoomSharpness=11f;
            rig.orthographicSize=Mathf.Clamp(rig.orthographicSize,rig.minSize,rig.maxSize);
        }

        static void EnsureModels() {
            Directory.CreateDirectory(FullPath(ModelDir)); int changed=0;
            foreach (Source s in Sources) {
                string assetPath=ModelDir+"/"+s.file, full=FullPath(assetPath);
                ...
                try {
                    using (WebClient wc=new WebClient()) {
                        ...
                        File.WriteAllText(full,StripMaterials(wc.DownloadString(s.url))); changed++;
                    }
                } catch (Exception ex) { Debug.LogWarning("V32 asset download failed for "+s.file+": "+ex.Message); }
            }
            if (changed>0) AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
            foreach (Source s in Sources) { string p=ModelDir+"/"+s.file; if (File.Exists(FullPath(p))) AssetDatabase.ImportAsset(p,ImportAssetOptions.ForceSynchronousImport|ImportAssetOptions.ForceUpdate); }
        }

        static string StripMaterials(string text) => string.Join("\n",text.Replace("\r\n","\n").Replace('\r','\n').Split('\n')
        ...
        static GameObject PlaceModel(GameObject asset,Transform parent,string name,Vector3 pos,float yaw,float targetHeight,Material mat) {
            ...
            Bounds b=BoundsOf(go); go.transform.localScale=Vector3.one*(targetHeight/Mathf.Max(.01f,b.size.y));
        ...
        static void Puddle(Transform parent,Vector3 pos,float yaw,float length,float width,Material mat) {
            GameObject g=GameObject.CreatePrimitive(PrimitiveType.Cylinder); g.name="roadside puddle"; g.transform.SetParent(parent,false);
            g.transform.position=pos; g.transform.rotation=Quaternion.Euler(0f,yaw,0f); g.transform.localScale=new Vector3(length,.006f,width);
            g.GetComponent<Renderer>().sharedMaterial=mat; UnityEngine.Object.DestroyImmediate(g.GetComponent<Collider>());
        }

        static bool SafeForNature(Vector3 p,List<RosvikOsmV15.Way> ways,float clearance) => !InsideBuilding(p,ways) && !InsidePitch(p,ways) && DistanceToRoad(p,ways)>clearance;
        ...
        static float DistanceToRoad(Vector3 p,List<RosvikOsmV15.Way> ways) { float best=float.MaxValue; foreach (var w in ways) { if (string.IsNullOrEmpty(w.Tag("highway")) || w.Nodes.Count<2) continue; for (int i=0;i<w.Nodes.Count-1;i++) best=Mathf.Min(best,Vector3.Distance(Flat(p),Flat(ClosestPoint(p,w.Nodes[i].Pos,w.Nodes[i+1].Pos)))); } return best; }
        ...
        static Material Mat(string name,Color color,float smooth) {
            ...
            m.shader=s; m.color=color; if (m.HasProperty("_BaseColor")) m.SetColor("_BaseColor",color); if (m.HasProperty("_Color")) m.SetColor("_Color",color);
