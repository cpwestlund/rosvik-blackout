...
# - first residential edge beyond the school/sport cluster
# - collision pass for large decorative props that previously behaved like ghosts
# - searchable everyday containers and a small persistent backpack
# - usable flashlight as the first general-purpose exploration tool
...
func _enter_tree() -> void:
	...
	# Register the new houses before any inherited placement validation runs.
...
# -------------------------------------------------------------------------
# RESIDENTIAL EDGE: A FIRST STEP OUTSIDE THE CIVIC CLUSTER
# -------------------------------------------------------------------------
func _build_expansion11() -> void:
	...
	# A modest sign makes this read as continuation of the village, not another arena.
...
func _build_residential_road11() -> void:
	# The old west road ends at (-105,-30). Extend from that endpoint instead of
	# drawing a new rectangle across the existing road. A dedicated turn patch joins
	# the new north-south street, preventing the broken cross / overlap look.
	var road_mat: Material = asphalt_mat
	...
	# Driveways stay off the street and stop before the houses.
	for z_value: float in [-10.0,8.0,25.0]:
		var drive: MeshInstance3D = _box(Vector3(5.2,0.045,3.15),road_mat,Vector3(-106.5,0.052,z_value),_residential11_root)
		drive.rotation.y = 0.0

	# Snow edge / pedestrian strip on the house side only. No coplanar layer under road.
...
func _build_house11(pos: Vector3, color: Color, variant: int) -> void:
	...
	# Street-facing facade points west toward the new road.
	...
	for z_w: float in [-1.55,0.05]:
		var frame: MeshInstance3D = _box(Vector3(0.12,1.16,1.42),dark_mat,Vector3(-4.78,1.68,z_w),root)
		_box(Vector3(0.135,0.96,1.22),glass_mat,Vector3(-4.84,1.68,z_w),root)

	# Porch, two steps and a small warm exterior lamp.
	...
	# Garage/lean-to varies between houses to avoid copy-paste silhouettes.
	if variant != 1:
...
func _add_house_detail11(root: Node3D, variant: int) -> void:
	...
	var bin_m: StandardMaterial3D = _mat(Color("3f5550"),0.94)

	# Mailbox at driveway edge.
	...
	# Two wheelie bins and a deliberately solid woodpile.
	for i: int in range(2):
		var x: float = 4.05-float(i)*0.72
		_solid_box(Vector3(0.58,0.88,0.56),bin_m.darkened(float(i)*0.04) if bin_m is StandardMaterial3D else bin_m,Vector3(x,0.44,3.76),root)
	...
	if variant == 2:
		# Half-buried children's sled: small visual storytelling, non-blocking.
...
# -------------------------------------------------------------------------
# COLLISION PASS: DECORATION THAT LOOKS SOLID SHOULD FEEL SOLID
# -------------------------------------------------------------------------
func _build_collision_pass11() -> void:
	...
	# Storytelling 08 exterior clusters.
	...
	# Schoolyard furniture which previously read as physical but could be walked through.
	...
	# Generator tool crate from milestone 10.
...
# -------------------------------------------------------------------------
# SEARCHABLE EVERYDAY LOOT
# -------------------------------------------------------------------------
func _build_searchables11() -> void:
	# Expedition cleaning/storage cabinet.
	...
	# Sporthall service chest.
	_add_search_cabinet11(_gameplay_root,Vector3(47.0,0.0,33.25),0.0,"SERVICEFÖRRÅD",["aa","konserv"])

	# Residential garage cabinet contains the flashlight: an exploration reward,
	# not a required item for the blackout mission.
	_add_search_cabinet11(_residential11_root,Vector3(-104.55,0.0,8.6),PI/2.0,"GARAGESKÅP",["ficklampa","aa"])

	# A small abandoned-car boot cache near the new street.
	_add_car(Vector3(-106.6,0.0,24.4),Color("665d54"),PI/2.0,true)
	var boot: Node3D = Node3D.new()
	boot.name = "AbandonedCarBoot11"
	boot.position = Vector3(-106.2,0.0,24.4)
	_expansion11_root.add_child(boot)
	_register_searchable11(boot,"BAGAGELUCKA",["konserv","tandstickor"] ,2.0)

func _add_search_cabinet11(parent: Node3D, pos: Vector3, yaw: float, title: String, loot: Array[String]) -> void:
	var root: Node3D = Node3D.new()
...
func _register_searchable11(node: Node3D, title: String, loot: Array[String], radius: float) -> void:
	_gameplay_interactables.append({
		"node":node,
		"kind":"search11",
		"title":title,
		"radius":radius,
...
# -------------------------------------------------------------------------
# FLASHLIGHT: FIRST GENERAL EXPLORATION TOOL
# -------------------------------------------------------------------------
func _build_player_flashlight11() -> void:
	...
	# SpotLight3D points toward local -Z; player locomotion faces +Z.
