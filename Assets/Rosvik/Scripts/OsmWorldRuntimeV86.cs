...
namespace Rosvik.Blackout {
    ...
    public sealed class CutawayTargetV86 {
        ...
        public Collider[] insideVolumes = Array.Empty<Collider>();
    }
    ...
    public sealed class OsmWorldRuntimeV86 : MonoBehaviour {
        ...
        void Apply(bool force) {
            ...
            foreach (CutawayTargetV86 t in cutaways) {
                ...
                bool inside = false;
                if (t.insideVolumes != null && t.insideVolumes.Length > 0) {
                    foreach (Collider c in t.insideVolumes) {
                        if (!c || !c.enabled) continue;
                        Vector3 closest = c.ClosestPoint(p);
                        if ((closest - p).sqrMagnitude < 0.0004f) { inside = true; break; }
                    }
                } else {
